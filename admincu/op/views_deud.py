...
def deud_ver(request, pk):
	...
	# detectamos si es una deuda de proveeduría
	es_proveeduria = gastoDeuda.filter(gasto__es_proveeduria=True).exists()

	compras = None
	if es_proveeduria:
		# gracias al related_name='compras' en Compra_Producto.deuda
		compras = deuda.compras.all().select_related('producto')

	context = {
		'deuda': deuda,
		'gastoDeuda': gastoDeuda,
		'es_proveeduria': es_proveeduria,
		'compras': compras,
	}
	return render(request, 'deudas/ver.html', context)
